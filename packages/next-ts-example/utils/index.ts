 ...
 */
export function getTabClassName(tab: string, good: boolean, top: boolean): string {
  let className = ''
 ...
 * 获取文字标签
 */
export function getTabStr(tab: string, good: boolean, top: boolean): string {
  let str = ''
...
export function fmtDate(date: Date, fmt: string): string {
  ...
  for (let k in o) {
    if (new RegExp('(' + k + ')').test(fmt)) {
      let replaceValue = RegExp.$1.length == 1 ? o[k].toString() : ('00' + o[k]).substr(('' + o[k]).length)
